...
export default {
    loadFileLists(){
        ...
        let that = this
        setupCache(Axios.create()).get('filePath?size=20&skip='+((this.tableData.length>0)?this.tableData.length-1:0))
        .then(function (response) {
            const load = async (data) => {
                ...
                for (let index = 0; data && index < data.length; index++) {
                    ...
                    let result2 = []
                    await setupCache(Axios.create()).get('danmuCountPerMin?ref='+element.path)
                    .then(function (response) {
